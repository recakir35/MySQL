...
create table personel(id char(4), isim varchar(40), maas int);
...
-- BETWEEN : Belirttiğimiz iki veri arasındaki bilgileri listeler
...
select * from personel where isim between 'Derya Soylu' and 'Yavuz Bal';  -- D ve Y harfleri arasındakileri getirdi.


-- in : aynı sutundaki değişkenler  Birden fazla ifade ile tanımlayabileceğimiz durularda kullanılır--
...
select * from kelimeler where regexp_like (kelime, 'ot|at');  -- default i vardır
...
select * from kelimeler where regexp_like (kelime, '^ho|^hi','c'); 
...
select * from kelimeler where regexp_like (kelime, 'h[a-zA-Z0-9]t','c'); -- h 1. harf, [] 2. harf, t 3. harf
