...
CREATE TABLE calisanlar (id int, isim VARCHAR(50), sehir VARCHAR(50), maas int, isyeri VARCHAR(20));
...
CREATE TABLE markalar(marka_id int, marka_isim VARCHAR(20), calisan_sayisi int);
...
select isyeri, isim, maas from calisanlar where isyeri in(select marka_isim from markalar where calisan_sayisi>15000);
...
-- round(avg(maas),2) : virgülden sonra iki basamak gösterir.
-- as : yapılan sorguyu isimlendiriyoruz.
...
select marka_id, marka_isim,(select  count(sehir) from calisanlar where marka_isim=isyeri) as sehir_adedi from markalar;
